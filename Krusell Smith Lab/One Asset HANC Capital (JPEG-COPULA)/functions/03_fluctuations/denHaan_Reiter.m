...
c_aux = myidct(c_aux,1); % do dct-transformation
c_aux = myidct(c_aux,2); % do dct-transformation
...
options=optimset('Display','off','TolFun',1e-7,'TolX',1e-7);
...
% Wage Rate
Wminus =  TFP *par.alpha.* (Kminus./(par.N)).^(1-par.alpha);
% Return on Capital
Rminus =  TFP *(1-par.alpha).* ((par.N)./Kminus).^(par.alpha)  - par.delta;
%Price of Capital
