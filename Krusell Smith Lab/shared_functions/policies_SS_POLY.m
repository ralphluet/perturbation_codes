...
Euler = @(K)(EulerError(K,Y,mutil,par,mpar,gri,prob,meshes)); % Define Euler Error Function inline
...
function [EulerE] = EulerError(Theta,Y,mutil,par,mpar,gri,prob,meshes)
...
for z=1:mpar.nh    
    kprime(:,z) = polyval(Theta(:,z),grid.k); % p(d_t)
end
