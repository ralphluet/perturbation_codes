...
@flattenable @prior @latex_label @label @with_kw struct ModelParameters{T} # true/false denotes whether estimated
	...
	ρ_h::T = 0.9   | "rho" | L"\rho" |  _  | false # autocorrelation income shock
	σ_h::T = 0.25   | "sigma" | L"\sigma" |  _  | false # std of income shocks (steady state)
	...
	α::T = 0.33  | "alpha" | L"\alpha" |  _  | false # capital share
	δ_0::T = 0.025  | "delta" | L"\delta" |  _  | false # depreciation rate
	δ_s::T = 100.0 | "delta_s" | L"\delta_s" | Gamma(gamma_pars(5.0, 2.0^2)...) | false # depreciation rate increase (flex utilization)
	ϕ::T = 0.0   | "phi" | L"\phi" | Gamma(gamma_pars(4.0, 2.0^2)...)    | true # Capital adjustment costs
	μ::T = 1.05 | "mu" | L"\mu" |  _  | false # markup
	κ::T = 10000.0  | "kappa" | L"\kappa" | Gamma(gamma_pars(0.1, 0.02^2)...) | true # Price adjustment costs (in terms of Calvo probs.)
	μw::T = 1.0 | "mu_w" | L"\mu_w" |  _  | false # markup
	κw::T = 10000.0 | "kappa_w" | L"\kappa_w" | Gamma(gamma_pars(0.1, 0.02^2)...) | true # Price adjustment costs (in terms of Calvo probs.)
	...
	Rbar::T = (π*(1.125.^0.25) .- 1.0) | "Rbar" |  L"\bar R"  |  _  | false # borrowing wedge in interest rate
	...
	ρ_Gshock::T = 0.0 | "rho_Gshock" | L"\rho_G" | Beta(beta_pars(0.5, 0.2^2)...) | true # Pers. in structural deficit
