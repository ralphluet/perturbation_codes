function FSYSNODUAL(X::AbstractArray, XPrime::AbstractArray, Xss::Array{Float64,1}, m_par::ModelParameters,
    ...
    # cum_aux = (cum_aux2(:)+StateSS((mpar.nm+mpar.nh)+(1:NN)));
    ...
    Copula1(x::AbstractVector,y::AbstractVector,z::AbstractVector) = mylinearinterpolate3(cum_zero[:,end,end],cum_zero[end,:,end],cum_zero[end,end,:],
    cum_zero, x, y, z)


    CDF_joint     = Copula1(CDF_m[:], CDF_k[:], CDF_y[:]) # roughly 5% of time
    # CDF_AUX=zeros(eltype(θD),n_par.nm+1,n_par.nk+1, n_par.ny+1);
    # CDF_AUX[2:end,2:end,2:end] = CDF_joint
    ##### requires Julia 1.1
    ...
    inc =[  GHHFA.*(1.0 .- τlev).*((n_par.mesh_y/H) .*mcw.*w.*N).+
    ((1.0 .- mcw).*w.*N).*(1.0 .- τlev),# labor income (NEW)
    (r .- 1.0).* n_par.mesh_k, # rental income
    eff_int .* n_par.mesh_m, # liquid asset Income
    n_par.mesh_k .* q,
    (1.0 .- τlev).*(mcw.*w.*N.*n_par.mesh_y./ H)./(m_par.γ+1),
    (1.0 .- τlev).*((n_par.mesh_y/H) .*mcw.*w.*N)] # capital liquidation Income (q=1 in steady state)
    inc[1][:,:,end].= (1.0 .- τlev).*(n_par.mesh_y[:,:,end] .* profits) # profit income net of taxes
    inc[5][:,:,end].= 0.0
    inc[6][:,:,end].= (1.0 .- τlev).*(n_par.mesh_y[:,:,end] .* profits) # profit income net of taxes
    ...
    Vk_new,Vm_new = updateV(EVkPrime ,c_a_star, c_n_star, m_n_star, r - 1.0, q, m_par.λ, Π) # update expected marginal values time t
    ...
    # CDF_AUX2=zeros(n_par.nm+1,n_par.nk+1, n_par.ny+1);
    # CDF_AUX2[2:end,2:end,2:end] = CDF_joint
    ...
    # Copula = griddedInterpolant({[0; cum_m],[0; cum_h']},cum_zero,'spline');
    #
    #
    ...
    # aux = mydct(aux(1:end-1,1:end-1),1,DC3); % do dct-transformation
    # aux = mydct(aux,2,DC4); % do dct-transformation
    # DC=aux(:);
    #
    # RHS(COPind) = (DC(indexCOPdct)); % Write Marginal Utility to RHS of F


    distr_m_act, distr_k_act, distr_y_act, share_borroweract, GiniWact, I90shareact, GiniXact, #=
    =# sdlogxact, P9010Cact, GiniCact, sdlgCact, P9010Iact, GiniIact, sdlogyact, w90shareact, P10Cact, P50Cact, P90Cact =
    distrSummaries(distr,c_a_star, c_n_star, n_par, inc, m_par)
